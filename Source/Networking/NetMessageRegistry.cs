...
public class NetMessageRegistry
{
    ...
    public void Configure(
        Action<CSteamID, byte[], SendMode> sendToClient,
        Action<byte[], SendMode> broadcast,
        Func<CSteamID?> getHostId)
    {
    ...
    public void Register<T>(byte typeId, Action<CSteamID, T> handler, bool checkTimestamp = false)
        where T : INetMessage, new()
    {
        if (_idToRegistration.ContainsKey(typeId))
        {
            throw new InvalidOperationException($"Message type ID {typeId} is already registered");
        }
        ...
        UmcLogger.Debug($"Registered message: {typeof(T).Name} (ID={typeId})");
    }
    ...
    public void Send<T>(T message, SendMode mode, SendTarget target) where T : INetMessage
    {
        ...
        {
            UmcLogger.Error($"Cannot send unregistered message type: {typeof(T).Name}");
            return;
        }
        ...
        switch (target.Type)
        {
            ...
            case SendTargetType.Host:
                ...
                if (hostId.HasValue)
                {
                    _sendToClient?.Invoke(hostId.Value, data, mode);
                }
                else
                {
                    UmcLogger.Warn("Cannot send to host - host ID not available");
                }
                break;
    ...
    public void SendTo<T>(T message, CSteamID target, SendMode mode = SendMode.Reliable) where T : INetMessage
    {
        Send(message, mode, SendTarget.ToClient(target.m_SteamID));
    }

    public void Broadcast<T>(T message, SendMode mode = SendMode.Reliable) where T : INetMessage
    {
        Send(message, mode, SendTarget.Broadcast);
    }

    public void SendToHost<T>(T message, SendMode mode = SendMode.Reliable) where T : INetMessage
    {
        Send(message, mode, SendTarget.Host);
    }
    ...
    public bool HandleRawMessage(CSteamID sender, byte[] data)
    {
        ...
        try
        {
            ...
            {
                UmcLogger.Warn($"Received unknown message type ID: {typeId}");
                return false;
            }

            // Check timestamp for state updates
            if (registration.CheckTimestamp)
            {
                if (_lastTimestamps.TryGetValue(sender, out var lastTimestamp) && timestamp < lastTimestamp)
                {
                    return false; // Old message, skip
                }
    ...
    public void ClearPeerState(CSteamID peer)
    {
        _lastTimestamps.Remove(peer);
    }
