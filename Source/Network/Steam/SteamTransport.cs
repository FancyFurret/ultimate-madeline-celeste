...
/// <summary>
/// Low-level Steam P2P transport. Handles sending/receiving raw bytes.
/// </summary>
public class SteamTransport
{
    ...
    public const float StateUpdateInterval = 1f / 30f;
    ...
    public void Initialize()
    {
        ...
        {
            UmcLogger.Warn("Cannot initialize SteamTransport - Steam not initialized");
            return;
        }
        ...
        UmcLogger.Info("SteamTransport initialized");
    }

    public void Shutdown()
    {
        if (SteamLobby.Instance?.InLobby == true)
        {
            foreach (var member in SteamLobby.Instance.LobbyMembers)
            {
                if (!member.IsLocal)
        ...
        UmcLogger.Info("SteamTransport shut down");
    }
    ...
    public void SendTo(CSteamID userId, byte[] data, SendMode mode)
    {
        ...
        if (!SteamNetworking.SendP2PPacket(userId, data, (uint)data.Length, sendType, channel))
            UmcLogger.Warn($"Failed to send packet to {SteamManager.GetPlayerName(userId)}");
    }

    public void Broadcast(byte[] data, SendMode mode)
    {
        if (SteamLobby.Instance?.InLobby != true) return;
